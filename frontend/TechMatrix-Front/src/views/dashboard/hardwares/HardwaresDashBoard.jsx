...
import { deleteCpu, deleteGpu } from "../../../api/api";
...
export default function HardwaresDashBoard() {
    ...
    const handleSubmit = async (e) => {
        switch (e.currentTarget.id) {
            case "cpu":
                if (!confirm("DELETE " + e.currentTarget.name + " ?")) {
                    return;
                }
                const responseCpu = await deleteCpu(e.currentTarget.value);
                window.location.reload();
                return responseCpu.json();
            case "gpu":
                if (!confirm("DELETE " + e.currentTarget.name + " ?")) {
                    return;
                }
                const responseGpu = await deleteGpu(e.currentTarget.value);
                window.location.reload();
                return responseGpu.json();
        }
    };
        ...
        <>
            <main class="admin">
                ...
                <p>
                    ...
                    <Link to={`/dashboard/hardwares/new/cpu`}>
                        [+] Create new cpu
                    </Link>
                </p>
                ...
                <section class="gpuSectionDashboard">
                    <p>
                        &gt; Initializing Editmanager-gpu <br />
                        <Link to={`/dashboard/hardwares/new/gpu`}>
                            [+] Create new gpu
                        </Link>
                    </p>
                    <h1>Gpu</h1>
